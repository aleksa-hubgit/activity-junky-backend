...
func (s *AuthService) Login(ctx context.Context, lr LoginRequest) (*LoginResponse, error) {
	resp, err := s.httpClient.Get(fmt.Sprintf("http://user-service:8080/users/%s", lr.EmailOrUsername))
	if err != nil {
		fmt.Println(err)
		return nil, err
	}
	...
	if userResp.Password != lr.Password || (userResp.Username != lr.EmailOrUsername && userResp.Email != lr.EmailOrUsername) {
		return nil, errors.New("invalid credentials")
	}
	...
	s.database.CreateToken(ctx, data.CreateTokenParams{ Tokenstring: refreshTokenString, Username: userResp.Username})
	return &LoginResponse{Token: tokenString, Email: userResp.Email, Username: userResp.Username, UserType: userResp.UserType}, nil
}
